...
class TestFactExtractorFeatureExtractor(unittest.TestCase):
    ...
    def test_feature_for(self):
        extractor = feature_extractors.FactExtractorFeatureExtractor('en')
        self.assertEqual(extractor.feature_for('word1', 'pos', 3, True), 0)
        self.assertEqual(extractor.feature_for('word2', 'lemma', -2, True), 1)
        self.assertEqual(extractor.feature_for('WoRd1', 'POs', 3, True), 0)

    def test_extract_features_no_window(self):
        extractor = feature_extractors.FactExtractorFeatureExtractor('en', 0)
        _, f1 = extractor.extract_features(add_unknown=True, **self.sentences_data[0])
        _, f2 = extractor.extract_features(add_unknown=True, **self.sentences_data[1])
    ...
    def test_extract_features_window(self):
        ...
        _, feat = extractor.extract_features(add_unknown=True, **self.sentences_data[1])
        self.assertEqual(len(feat[2][0]), 3 * (2 * window + 1))

    def test_feature_labels(self):
        extractor = feature_extractors.FactExtractorFeatureExtractor('en')
        _, tokens = extractor.extract_features(add_unknown=True, **self.sentences_data[0])
    ...
    def test_get_training_set(self):
        extractor = feature_extractors.FactExtractorFeatureExtractor('en')
        extractor.process_sentence(add_unknown=True, **self.sentences_data[0])
        extractor.process_sentence(add_unknown=True, **self.sentences_data[1])
    ...
    def test_unknown_token(self):
        ...
        self.assertEqual(extractor.feature_for('a', 'b', 12, add_unknown=False),
                         extractor.feature_index.index(extractor.unk_token))
