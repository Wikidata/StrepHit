...
class EnglishArtistsSpider(Spider):
        ...
        for each in response.xpath(
            ...
            if content and content[0].xpath('local-name()').extract() == ['span']:
                ...
                item = WebSourcesCorpusItem(
                    ...
                    bio=utils.clean_extract(each, './/text()', sep=' '),
                )

                if each.xpath('./i'):
                    item['other'] = {
                        'profession': utils.clean_extract(each, './i//text()')
                    }
                
                assert item['name'] and len(item['name']) > 3
            elif item:
                item['bio'] += '\n' + utils.clean_extract(each, './/text()', sep=' ')
    ...
    def finalize(self, item):
        ...
        item['bio'] = utils.clean(item['bio'])
        item['name'] = utils.clean(','.join(item['name'].split(',')[:-1]))
        return item

    def text_from_node(self, node):
        return (utils.clean_extract(node, './/text()', sep=' ')
                if node.xpath('local-name()').extract()
                else utils.clean(node.extract()))
